...
def find_rectangles_template_match(screenshot, asset, threshold=0.3, return_scores=False, mask=False):
    ...
    if mask is True and len(asset.shape) == 2:
        score = cv.matchTemplate(screenshot, asset, cv.TM_CCORR_NORMED, mask=asset)
    else:
...
def add_targets(screenshot, targets, marker_color=(0, 0, 0)):
    ...
    for target in targets:
        cv.drawMarker(screenshot, (target[0], target[1]), marker_color, marker_type, markerSize=50)
